...
public class MainListActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		GenericUtil.currentActivity = MainListActivity.class.toString();
...
//		}
	...
	public void doInitData() {
		...
		if (categories.size() != 0 && categories != null) {
			...
			sce1LeftCategoryName.setText(categories.get(currentCategoryIndex).getCategoryName());
			...
			Toast.makeText(this, getResources().getString(R.string.message_notfoundata), Toast.LENGTH_SHORT).show();
		}
	}
	...
	public List<CategoryModel> getDishsByCategory(long categoryId) {
		dishDAO.open();
		List<DishModel> dishs = dishDAO.getDishsByCategoryId(categoryId);
		List<CategoryModel> categories = new ArrayList<CategoryModel>(groupingDishInCategory(dishs).values());
		dishDAO.close();
		return categories;
	}
	...
	public void initCurrentStatus() {
		...
		long refsCategoryId = sharedPreferences.getLong("scr1CurrentCategoryId", 0);
		...
		try {
			...
			dishs = (dishs.size() > 0) ? dishs : getDishsByCategoryId(currentCategoryModel.getCategoryId());
			currentCategoryModel.setDishs(dishs);
			currentDishModel = currentCategoryModel.getDishs().get(currentDishIndex);
			currentDishModel.setAvatarImg(getAvatarOfDish(currentDishModel.getDishID()));
		} catch (Exception e) {
	...
	public void onClickLeftCategoryArrow(View view) {
		...
		if (this.categories.size() > 0) {
			if (id == R.id.sce1_left_category_left) {
				currentCategoryIndex = (currentCategoryIndex == 0) ? (categories.size() - 1) : currentCategoryIndex - 1;
			} else {
				currentCategoryIndex = (currentCategoryIndex == categories.size() - 1) ? (0) : currentCategoryIndex + 1;
			}

			currentCategoryModel = this.categories.get(currentCategoryIndex);
			currentCategoryModel.setDishs(getDishsByCategoryId(currentCategoryModel.getCategoryId()));

			sce1LeftCategoryName.setText(currentCategoryModel.getCategoryName());
	...
	public void updateSce1LeftDishList() {
		if (currentCategoryIndex >= 0 && currentCategoryIndex < this.categories.size()) {

			leftDishListView = new ArrayList<LinearLayout>();
			leftListDishLayout.removeAllViews();

			LinearLayout dishItemLayout;
			LinearLayout.LayoutParams itemLayout = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,
					LayoutParams.WRAP_CONTENT);
			LinearLayout.LayoutParams itemNameLayout = new LinearLayout.LayoutParams(190, LayoutParams.WRAP_CONTENT);
			LinearLayout.LayoutParams itemPriceLayout = new LinearLayout.LayoutParams(100, LayoutParams.WRAP_CONTENT);
			...
			for (int i = 0; i < currentCategoryModel.getDishs().size(); i++) {
				...
				nameText.setTextAppearance(dishItemLayout.getContext(), android.R.style.TextAppearance_Medium);
				nameText.setLayoutParams(itemNameLayout);
				nameText.setText(currentCategoryModel.getDishs().get(i).getDishName());

				priceText = new TextView(dishItemLayout.getContext());
				priceText.setPadding(5, 0, 0, 0);
				priceText.setTextAppearance(dishItemLayout.getContext(), android.R.style.TextAppearance_Medium);
				priceText.setLayoutParams(itemPriceLayout);
				priceText.setText(currentCategoryModel.getDishs().get(i).getReferPrice() + "");
	...
	public void updateSce1LeftDishSelected(int position) {
		try {
			if (this.currentCategoryModel.getDishs().get(position) != null) {
				LinearLayout currentLayout = leftDishListView.get(currentDishIndex);
	...
	public void updateScr1RightDishSelected() {
		...
		this.currentDishModel = this.currentCategoryModel.getDishs().get(this.currentDishIndex);
		/*----Set Avatar for dish ---------------*/
		...
		String numOfDinerText = (numOfDiner < 10) ? "0" + numOfDiner : numOfDiner + "";
	...
	public void menuOnClick(View view) {
		// TODO Auto-generated method stub
		MenuDialog menuDiaglog = MenuDialog.newInstance(getResources().getString(R.string.menu_function_title),
	...
	public void saveCurrentState() {
		...
		if (this.currentCategoryModel != null && this.currentDishModel != null) {
			editor.putLong("scr1CurrentCategoryId", this.currentCategoryModel.getCategoryId());
