...
public class CategoryPersistenceImpl extends BasePersistenceImpl<Category>
	...
	public List<iRestads.model.Dish> getDishs(long pk, int start, int end,
		...
		if (list == null) {
			...
			try {
				...
				q.addEntity("dishstore_Dish", iRestads.model.impl.DishImpl.class);
	...
	private static final String _SQL_GETDISHS = "SELECT {dishstore_Dish.*} FROM dishstore_Dish INNER JOIN dishstore_Category ON (dishstore_Category.categoryId = dishstore_Dish.categoryId) WHERE (dishstore_Category.categoryId = ?)";
	private static final String _SQL_GETDISHSSIZE = "SELECT COUNT(*) AS COUNT_VALUE FROM dishstore_Dish WHERE categoryId = ?";
	private static final String _SQL_CONTAINSDISH = "SELECT COUNT(*) AS COUNT_VALUE FROM dishstore_Dish WHERE categoryId = ? AND dishId = ?";
