...
public class CategoryAdsPersistenceImpl extends BasePersistenceImpl<CategoryAds>
	...
	public List<irestads.model.AdsItem> getAdsItems(long pk, int start,
		...
		if (list == null) {
			...
			try {
				...
				q.addEntity("dishsstore_AdsItem",
					irestads.model.impl.AdsItemImpl.class);
	...
	private static final String _SQL_GETADSITEMS = "SELECT {dishsstore_AdsItem.*} FROM dishsstore_AdsItem INNER JOIN dishsstore_CategoryAds ON (dishsstore_CategoryAds.categoryAdsId = dishsstore_AdsItem.categoryAdsId) WHERE (dishsstore_CategoryAds.categoryAdsId = ?)";
	private static final String _SQL_GETADSITEMSSIZE = "SELECT COUNT(*) AS COUNT_VALUE FROM dishsstore_AdsItem WHERE categoryAdsId = ?";
	private static final String _SQL_CONTAINSADSITEM = "SELECT COUNT(*) AS COUNT_VALUE FROM dishsstore_AdsItem WHERE categoryAdsId = ? AND adsItemId = ?";
